...
class WatchHistory:
    ...
    def parse_watch_history_data(self):
        ...
        # English:
        if self.watch_df['title'].str.startswith('Watched').any():
            self.watch_df['title'] = self.watch_df['title'].str[8:]
        #Hebrew:
        if self.watch_df['title'].str.startswith('צפית בסרטון').any():
            self.watch_df['title'] = self.watch_df['title'].str[12:]
        ...
        # Remove all rows with titleUrl == 0
        self.watch_df = self.watch_df[self.watch_df.titleUrl != 0]
